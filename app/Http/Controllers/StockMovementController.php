...
use Illuminate\Http\Request; // Your newly seeded model!
...
class StockMovementController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            'tx_id' => ['required', 'string', 'max:255', 'unique:stock_movements,tx_id'],
            'date' => ['required', 'date'],
            'part_id' => ['required', 'string', 'exists:items,id'],
            'type' => ['required', 'in:Stock-In,Stock-Out,Warehouse Transfer,Product Return'],
        ...
        try {
            ...
            StockMovement::create([
                ...
                'date' => $validated['date'],
                'item_id' => $validated['part_id'], // Map frontend 'part_id' to DB 'item_id'
                'type' => $validated['type'],
                'qty' => $validated['qty'],
            ...
            return response()->json([
                'success' => false,
                'message' => 'Unable to create the stock movement.',
            ], 500);
        }
    }
    ...
    public function updateStatus(Request $request, $txId)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Unable to update the stock movement status.',
            ], 500);
        }
    }
}
