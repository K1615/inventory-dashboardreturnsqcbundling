...
### Real logged-in user everywhere
- Removed the fake "Admin 1-4" dropdown and broken Logout button from every page.
- Real "Logged in as [name] [role]" + working Logout now shows at the bottom-left of the sidebar.
- Logs, requests, and approvals now show who actually did it, formatted as `Name(Role)` — no more fake or spoofable names.
